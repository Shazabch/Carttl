...
<div class="accordion-item report-card">
    <h2 class="accordion-header" id="headingTires">
        <button class="accordion-button card-header p-3 collapsed" type="button"
                data-bs-toggle="collapse" data-bs-target="#tiresCollapse"
                aria-expanded="false" aria-controls="tiresCollapse">
            <i class="fa-solid fa-circle-dot"></i>Tires
        </button>
    </h2>

    <div id="tiresCollapse" class="accordion-collapse collapse"
         aria-labelledby="headingTires" data-bs-parent="#reportAccordion">
        <div class="accordion-body card-body p-3">
            <div class="row g-3">

                @php
                    $getFieldImages = function($fieldName) use ($reportInView) {
                        return $reportInView->fields
                    ...
                    $allFields = [
                        ...
                        'tiresSize', 'spareTire', 'wheelsType', 'rimsSizeFront', 'rimsSizeRear'
                    ];
                @endphp

                {{-- Loop through all tire fields --}}
                @foreach($allFields as $index => $field)
                    @php
                        $data = $reportInView->{$field} ?? 'N/A';
                        $statusInfo = getStatusInfo($data);
                        $images = $getFieldImages($field);
                        $fieldId = 'tire-field-' . $index;
                    @endphp

                    <div class="col-12 col-md-4">
                        <div class="border rounded p-3 bg-white h-100 d-flex flex-column">
                            {{-- Clickable Label with Arrow AFTER name --}}
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <div class="item-label fw-semibold d-flex align-items-center"
                                     style="cursor: pointer;"
                                     data-bs-toggle="collapse"
                                     data-bs-target="#{{ $fieldId }}-images"
                                     aria-expanded="false">
                                    <i class="{{ $fieldIcons[$field] ?? 'fas fa-circle-notch' }} me-1"></i>
                                    {{ Str::of($field)->kebab()->replace('-', ' ')->title() }}
                                    @if($images->count() > 0)
                                        <span class="badge bg-primary ms-2 small">{{ $images->count() }}</span>
                                    @endif
                                    <i class="fa-solid fa-chevron-right field-toggle-arrow ms-auto"></i>
                                </div>
                            </div>
                            ...
                            <div class="flex-grow-1">
                                @if(is_array($data))
                                    <ul class="item-value-list mb-0 small">
                                        @foreach($data as $value)<li>{{ $value }}</li>@endforeach
                                    </ul>
                                @elseif($statusInfo['class'] !== 'item-value')
                                    <div class="status-pill {{ $statusInfo['class'] }} small">
                                        <i class="{{ $statusInfo['icon'] }}"></i>{{ $data }}
                                    </div>
                                @else
                                    <div class="item-value small">{{ $data }}</div>
                                @endif
                            </div>

                            {{-- Images Accordion --}}
                            @if($images->count() > 0)
                                <div class="collapse mt-3" id="{{ $fieldId }}-images">
                                    <div class="d-flex flex-wrap gap-1 p-2 bg-light rounded">
                                        @foreach($images as $img)
                                            <a href="{{ $img->path }}" target="_blank" class="text-decoration-none">
                                                <img src="{{ $img->path }}"
                    ...
                    <div class="border rounded p-3 bg-white">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="item-label fw-semibold d-flex align-items-center"
                                 ...
                                 data-bs-target="#{{ $fieldId }}-images"
                                 aria-expanded="false">
                                <i class="{{ $fieldIcons[$field] ?? 'fas fa-circle-notch' }} me-1"></i> Comments
                                @if($images->count() > 0)
                                    <span class="badge bg-primary ms-2 small">{{ $images->count() }}</span>
                                @endif
                                <i class="fa-solid fa-chevron-right field-toggle-arrow ms-auto"></i>
                        ...
                        <div>
                            ...
                            @elseif($statusInfo['class'] !== 'item-value')
                                <div class="status-pill {{ $statusInfo['class'] }} small" style="text-align:left;">
                                    <i class="{{ $statusInfo['icon'] }}"></i>{{ $data }}
                                </div>
                            @else
                                <div class="item-value small">{{ $data }}</div>
                ...
                <div class="accordion-item report-card" id="specs">
                    <h2 class="accordion-header" id="headingSpecs">
                        <button class="accordion-button card-header p-3 collapsed" type="button"
                            ...
                            aria-expanded="false" aria-controls="specsCollapse">
                            <i class="fa-solid fa-sliders"></i>Car Specs
                        </button>
                    </h2>
                    <div id="specsCollapse" class="accordion-collapse collapse"
                        aria-labelledby="headingSpecs" data-bs-parent="#reportAccordion">
                        <div class="accordion-body card-body p-3">
                            <div class="row g-3">
                                ...
                                $allFields = [
                                'parkingSensors', 'keylessStart', 'seats', 'cooledSeats', 'heatedSeats',
                                'powerSeats', 'viveCamera', 'sunroofType', 'drive', 'blindSpot',
                                'headsDisplay', 'premiumSound', 'carbonFiber', 'convertible', 'sideSteps',
                                'soft_door_closing'
                                ];
                                @endphp
                                ...
                                $statusInfo = getStatusInfo($data);
                                $images = $getFieldImages($field);
                                $fieldId = 'specs-field-' . $index;
                                $title = $field === 'soft_door_closing' ? 'Soft Door Closing' : Str::of($field)->kebab()->replace('-', ' ')->title();
                                @endphp
        ...
        <div class="accordion-body card-body p-3">
            <div class="row g-3">
                ...
                @php
                    ...
                    $allFields = [
                        'speedmeterCluster', 'headLining', 'seatControls', 'seatsCondition',
                        'centralLockOperation', 'sunroofCondition', 'windowsControl',
                        'cruiseControl', 'acCooling'
                    ];
                @endphp
                ...
                <div class="col-12 mt-3">
                    @php
                        $field = 'comment_section2';
                        $data = $reportInView->{$field} ?? 'N/A';
                        $statusInfo = getStatusInfo($data);
                        $images = $getFieldImages($field);
                        $fieldId = 'interior-comment';
                    @endphp
...
<div class="accordion-item report-card">
    <h2 class="accordion-header" id="headingSteering">
        <button class="accordion-button card-header p-3 collapsed" type="button"
                data-bs-toggle="collapse" data-bs-target="#steeringCollapse"
                aria-expanded="false" aria-controls="steeringCollapse">
            <i class="fa-solid fa-car-burst"></i>Steering, Suspension & Brakes
        </button>
    </h2>
    <div id="steeringCollapse" class="accordion-collapse collapse"
         aria-labelledby="headingSteering" data-bs-parent="#reportAccordion">
        <div class="accordion-body card-body p-3">
            <div class="row g-3">
                ...
                @php
                    ...
                    $allFields = [
                        'steeringOperation', 'wheelAlignment', 'brakePads',
                        'suspension', 'brakeDiscs', 'shockAbsorberOperation'
                    ];
                @endphp
                ...
                <div class="col-12 mt-3">
                    @php
                        $field = 'comment_section1';
                        $data = $reportInView->{$field} ?? 'N/A';
                        $statusInfo = getStatusInfo($data);
                        $images = $getFieldImages($field);
                        $fieldId = 'steering-comment';
                    @endphp
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('[data-bs-toggle="collapse"]').forEach(function (toggler) {
            ...
            const arrow = toggler.querySelector('.field-toggle-arrow');
            if (!arrow || !target) return;

            const collapseEl = document.querySelector(target);
            if (!collapseEl) return;

            collapseEl.addEventListener('shown.bs.collapse', () => arrow.classList.add('show'));
            collapseEl.addEventListener('hidden.bs.collapse', () => arrow.classList.remove('show'));
        });
    });
</script>
    </div>
