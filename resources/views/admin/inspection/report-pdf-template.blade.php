...
<head>
    ...
    <style>
        ...
        .report-section {
            ...
            margin-bottom: 15px;
            border-radius: 4px;
            page-break-inside: avoid; /* Important for preventing sections from splitting */
        }
        .section-title {
        ...
        .footer {
            ...
            font-size: 10px;
            color: #777;
            position: fixed; /* Optional: fixes footer to bottom of each page */
            bottom: 0;
        }
    </style>
</head>
        ...
        <div class="report-section">
            ...
            <div class="section-body">
                <table class="details-table">
                    <tr>
                        <td><div class="item"><span class="item-label">Make</span> <span class="item-value">{{ $report->make ?? 'N/A' }}</span></div></td>
                        <td><div class="item"><span class="item-label">Model</span> <span class="item-value">{{ $report->model ?? 'N/A' }}</span></div></td>
                    </tr>
                    <tr>
                        <td><div class="item"><span class="item-label">Year</span> <span class="item-value">{{ $report->year ?? 'N/A' }}</span></div></td>
                        <td><div class="item"><span class="item-label">VIN</span> <span class="item-value">{{ $report->vin ?? 'N/A' }}</span></div></td>
                    </tr>
                    <tr>
                        <td><div class="item"><span class="item-label">Odometer</span> <span class="item-value">{{ $report->odometer ?? 'N/A' }}</span></div></td>
                        <td><div class="item"><span class="item-label">Color</span> <span class="item-value">{{ $report->color ?? 'N/A' }}</span></div></td>
                    </tr>
                </table>
            </div>
        </div>
            ...
            @php
                ...
                foreach ($fields as $field) {
                    if (!empty($report->{$field})) {
                        $sectionData[$field] = $report->{$field};
                    }
                }
            @endphp
