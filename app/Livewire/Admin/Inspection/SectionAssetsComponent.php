...
class SectionAssetsComponent extends Component
{
    use WithFileUploads;

    public VehicleInspectionReport $vehicle;
    ...
    protected function rules()
    {
        return [
            ...
            'new_images.*' => 'image|max:10240', // 2MB Max per image
        ];
    }
    ...
    public function mount($inspectionId)
    {
        ...
        $this->vehicle = VehicleInspectionReport::with('images')->findOrFail($inspectionId);
    ...
    {
        $this->validate(['new_images.*' => 'image|max:10240']);
    ...
    public function updatedCoverImageId($photoId)
    {
        ...
        DB::transaction(function () use ($photoId) {
            ...
            VehicleInspectionImage::where('id', $photoId)->update(['is_cover' => true]);
        });
    ...
    {
        $imageToDelete = VehicleInspectionImage::find($imageId);
    ...
    {
        return view('livewire.admin.inspection.section-assets-component');
    }
}
